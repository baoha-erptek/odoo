...
The public Gearment docs mention `PRINT_LOCATION_CODE_WHOLE` and describe `location_code` + `url` but do **not** publish the exhaustive enum or field-naming rules. The renderer at [https://developers.gearment.com/api/api.order.v1.vendororderapi](https://developers.gearment.com/api/api.order.v1.vendororderapi) is reportedly a stub. The canonical source is the OpenAPI YAML (location TBD).
...
3. **Q1.3**: Can you provide a sample order payload captured from your **dashboard** (not our synthetic orders)? This would let us reverse-engineer the exact wire format.

   - Export or screenshot an order from Gearment dashboard UI + show us the JSON payload used to create it
   - Specifically, what does a multi-design (front + back) order look like in your system?
...
4. **Q2.1**: Our demo SKUs (`DEMO-T-*`) are being rejected by your API. Are these pre-registered in your **sandbox** catalog?
   ...
   - What are the valid product IDs / SKUs in the sandbox environment?
   - Can you pre-register a test SKU (e.g., `DEMO-TSHIRT-001`) for us in sandbox, or provide a list of available SKUs we can use?

5. **Q2.2**: When an order is rejected with HTTP 400, is the root cause always in the payload (schema/validation), or could it be a catalog lookup failure?

   - Does your validator emit different error codes / messages for "unknown SKU" vs. "malformed printing_options"?
   - Can you provide a full error response body (not just the truncated 400 status) for one of our rejected orders?
