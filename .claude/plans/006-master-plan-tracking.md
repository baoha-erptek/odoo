...
| P0-16 | Spec 005 sandbox — `EtsyOrderSyncer` against dev shop with VCR fixtures | Dev B | `doing` | P0-15 | Split into 3 sub-slices (planner 2026-04-28). **P0-16a landed** on `feature/006-master-plan-coding`: canonical `EtsyOrderPayload` + `EtsyChannelAdapter` Protocol + `HealthStatus` enum in `etsy_integration/services/`, 26 contract tests, 262 full-regression green. Module-home: `etsy_integration` (NOT core) per findings 2026-04-28. P0-16b (api adapter + ingestor + shop cursor field) and P0-16c (syncer + audit-mode + cassette-backed tests) next. Mock-and-fixture approach replaces `vcrpy` (owner decision 2026-04-28). |
