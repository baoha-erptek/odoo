...
| P0-07 | Spec 002 US6 implementation (migration wizard) — batch-resumable, per-500 savepoints, `last_processed_id` checkpoint | Dev A | `doing` | P0-05 | DA #3. **W3.2a backbone landed 2026-04-26** — model + 12 fields + anomaly quarantine (T036) + batched iteration with last_processed_id (T037) + sync.health (T038) + orchestration shell (T046) + view/menu/ACL (T047/T048/T049/T050). Fix-helper bodies (T039–T045) and apply_merges deferred to W3.2b. 12 new tests pass (99 total). |
