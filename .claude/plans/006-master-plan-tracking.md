...
| P0-07 | Spec 002 US6 implementation (migration wizard) — batch-resumable, per-500 savepoints, `last_processed_id` checkpoint | Dev A | `todo` | P0-05 | DA #3. Foundational pieces landed in W3.1 (savepoint helper + sync.health.report_run integration in import wizard); migration wizard reuses these patterns. |
