...
| P0-15 | Spec 005 sandbox — `EtsyApiClient` with rate limiter + retry/backoff | Dev B | `doing` | P0-14 | **RED phase landed 2026-04-27** on `feature/006-master-plan-coding` (commit `d84f92c3cbb`). 19 test methods across 5 classes defining complete contract: init validation, session headers, ping endpoint, 429-retry policy (Retry-After + backoff), 401-refresh policy (once per request, proactive within 60s). All tests intentionally FAIL (stub implementation). Next phase: GREEN implementation. Per-client token bucket for sandbox, shared bucket as Phase 1 refactor (architect Q3). |
