...
# Locate and source .env from repo root (overridable via ENV_FILE)
load_env() {
    local repo_root
    ...
    [ -n "${JIRA_SERVER_URL:-}" ]  || die "JIRA_SERVER_URL missing in $ENV_FILE"
    [ -n "${JIRA_PROJECT_KEY:-}" ] || die "JIRA_PROJECT_KEY missing in $ENV_FILE"
    [ -n "${JIRA_USER_EMAIL:-}" ]  || die "JIRA_USER_EMAIL missing in $ENV_FILE"
    [ -n "${JIRA_API_KEY:-}" ]     || die "JIRA_API_KEY missing in $ENV_FILE"
...
main() {
    ...
    local issue_type project summary jira_key status parent
    ...
    parent=$(get_field "$md_file" "parent")
