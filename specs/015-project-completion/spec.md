...
| **P-GEAR-AUTOCONFIRM** | Gearment draft → confirmed order (chargeable) | Gearment E2E audit 2026-07-05 | **awaiting owner** (options a/b/c DM'd 2026-07-05, msg 224) | **owner decision** | M | Odoo only creates DRAFT orders on Gearment; `adapter.confirm()` (/orders/draft/labeled) exists but is never called — production starts only after manual confirm in the Gearment dashboard. Needs owner sign-off (spends real money) + decide trigger (auto on PO confirm vs explicit button vs cron with cap). |
