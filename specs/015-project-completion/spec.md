...
| **MF-E2E-3b** | flow-3b Giao hàng Gearment dropship | **done** (2026-07-04, mock-fallback per exit criteria) | **Gearment validator** (vendor, Defect-2026-05-10-05) for live draft push | M | E2 keys LIVE (develop account). Runner `scripts/e2e_flow3b_dropship.py` **8/8 ×2** (`docs/engineering/uats/E2E_FLOW3B_DROPSHIP_2026-07-04.md`): live catalog + vendor-400 audited + LIVE HMAC webhook (**closes P0-18b2**) + Etsy push boundary; draft/quote legs via documented simulator (sandbox host dead). Playwright 8 ×2. Fix: webhook-env Etsy push sudo (mhf 1.0.27 — D-A primary trigger never worked live). OWNER: escalate printing_options validator to Gearment support before production dropship. |
