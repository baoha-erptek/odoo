...
- [ ] T023 [P] [US1] Extend `sale.order.line` in `models/sale_order_line.py` with rolled-up `design_status` (computed, store=True; lowest-of-children from `design_file_ids.state`) per data-model.md §3 — **deferred to P1-02** (depends on `design.file`).
