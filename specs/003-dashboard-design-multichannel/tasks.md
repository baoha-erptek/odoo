...
- [ ] T026 [US1] Implement compute method `_compute_is_duplicate_buyer` on `sale.order` (7-day window on partner_id + shipping address hash, excludes legitimate repeat customers per spec.md edge cases)
- [ ] T027 [P] [US1] Add overdue-approval marker computed field on `sale.order` (excludes final-state orders + non-etsy channels per FR-009 + spec.md edge cases)
- [ ] T028 [P] [US1] Add ACL entries for inline-edit fields (MP note, PIC, priority) — Marketing group write-allowed; BA group write-allowed
- [ ] T029 [US1] Add menu entry `Operations → Order Dashboard` in `views/menu.xml`
- [ ] T030 [P] [US1] Phase-2 test `tests/test_order_dashboard.py`: render-perf assertion (≤3s for 17K rows fixture), decoration logic per row type, inline-edit chatter audit
- [ ] T031 [US1] Phase-1 DB test in `tests/test_dashboards_db.py` verifying composite index `(sales_channel, has_pending_address_change)` exists post-install
...
- [ ] T063 [US5] Implement `design.file` model in `models/design_file.py` per data-model.md §6 with mail.thread + mail.activity.mixin
- [ ] T064 [US5] Implement constraints C-DF-001 through C-DF-005 (single-of order_id/order_line_id; storage_mode=small ⇒ size ≤ threshold; storage_mode=url ⇒ file_url non-empty; immutable history; re-upload pattern)
- [ ] T065 [US5] Override `ir.attachment.create` to enforce 10 MB ceiling on attachments linked to `design.file`/`sale.order`/`tracking.import.line` (FR-019)
...
- [ ] T071 [P] [US5] Create `views/design_file_views.xml` (form, list, kanban with 3 columns Chờ duyệt/Duyệt/Cần chỉnh lại); production-team-only ACL on drag-drop
...
- [ ] T076 [P] [US5] Phase-2 test `tests/test_design_file_lifecycle.py`: storage-mode enforcement, immutable history (parent_file_id chain), route state machine, idempotency_key dedup, stuck-route badge
- [ ] T077 [P] [US5] Phase-2 test for `design.print.batch` wizard: PDF generation, 24h cache TTL, only-approved-files constraint
- [ ] T078 [US5] Migration step: seed `design.file` from historical orders (FR-023) — `storage_mode='url'`, populated from `DESIGN_LINK_FRONT`/`DESIGN_LINK_BACK` columns (delegates to Spec 002's migration wizard, which calls into `multichannel_hub_core.design_file.create`)
