...
## P1-09 GREEN-phase implementation (2026-04-30)

Phase 3 (GREEN) completed. All 24 RED tests passing with ≥80% coverage.

### Pillow quality-tuning discovery

Binary-search JPEG quality (30-95) to hit max_size_kb threshold.
Initial attempt iterated 1-100 linearly; switched to binary search (10
iterations max, avg ~8 real). Code-sketch at
`design_thumbnail_generator.py:_find_quality`. Pattern reusable for
any size-constrained compression (PDF, WebM).

### GdriveUploader lazy-import pattern

`google.auth`, `google.oauth2`, `google.api_python_client` conditionally
imported only when `action_upload()` fires. If import fails, raises
`ImportError` downstream — avoids breaking module install on missing
google-cloud deps. Mirrors Pillow pattern (test for absence, catch
ImportError, warn via _logger). Tested via mock.patch.
`gdrive_uploader.py:9-22`.

### FR-017 RPC-gate 5th recurrence, action-method entry check

`design_file_upload_wizard.action_upload()` checks `has_group()` as first
line, raises `UserError` before any Drive interaction. View-level
`groups=` is necessary for UX but insufficient. Regression test
`test_non_production_team_cannot_call_action_upload_via_rpc` verifies
bypass detection (same pattern as P1-02a, P1-04, P1-03, P1-02b).

### Design-file lifecycle constraints (C-DF-006)

`storage_mode='gdrive'` + `gdrive_upload_state='uploaded'` ⟹
`gdrive_file_id` must be set (non-empty). Pending state must clear IDs.
Constraint implements ADR-006 §3 assertion: files have immutable storage
destination once assigned. Backfill handled via `init()` — no pre-existing
gdrive rows to migrate.

### ACL assignment pattern

`design.file.upload.wizard` inherits from production_team role
(read/write/create per `ir.model.access.csv`). Same group gates the
view button. Order-line context pre-filters to orders owned by
production_team user's shop(s) — no record-level rule override needed
today (future: may need O2M filtering if multi-shop assignment expands).

### Interop with existing views + wizards

`sale_order_line_views.xml` uses xpath position="before" (insert before
sheet) to avoid button-order assumptions. View inheritance inherits
from `sale.sale_order_line_view_form_readonly` (Odoo 19 standard).
Wizard target="new" opens as modal without back-navigation artifact.

All slots filled. Phase 7 (Document) and Phase 8 (Learn) complete.
Phase 9 (Land) deferred — accumulate on feature branch, merge post-W7
E2E.

### No ADR amendments

P1-09 GREEN output remains consistent with ADR-006 + ADR-012 as
audited in planner phase. No design changes post-planner.
