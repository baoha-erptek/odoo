...
| `storage_mode` | Selection: `small`/`url`/`gdrive` | Yes | `gdrive` for new uploads via wizard; `url` for historical seed | `small` = filestore Binary ≤10 MB; `url` = generic external URL (Etsy CDN or other); `gdrive` = Google Drive (ADR-006 §3 revised, ADR-012). Added in P1-09. |
...
| `file_url` | Char | No | — | Used when `storage_mode='url'` (Etsy CDN historical seed or other generic URL) |
| `gdrive_file_id` | Char | No | — | Required when `storage_mode='gdrive'`; Drive file ID returned by upload API. P1-09. |
| `gdrive_preview_url` | Char (computed `store=True`) | No | derived | `https://drive.google.com/file/d/{gdrive_file_id}/view`; depends on `gdrive_file_id`. P1-09. |
| `gdrive_folder_id` | Char | No | — | Destination folder ID on Drive (per-shop/year per ADR-006 §6). Cached on `etsy.shop.x_gdrive_design_folder_id` to avoid lookup per upload; stored per-row for audit. P1-09. |
| `gdrive_thumbnail` | Binary (`attachment=True`) | No | — | Local JPEG thumbnail ≤ 256 KB for Kanban preview when `storage_mode='gdrive'`. Generated by upload wizard via Pillow; empty on generation failure (non-fatal). P1-09. |
...
- C-DF-002: `storage_mode='small'` requires `design_file` non-empty AND `file_size <= multichannel_hub.large_file_threshold_bytes` (default 10 MB). GDrive-stored files bypass this cap (live on Drive, not in Odoo filestore). Implemented as both an `@api.constrains` AND an `ir.attachment.create` override (defence in depth, FR-019).
...
- C-DF-006: `storage_mode='gdrive'` requires `gdrive_file_id` non-empty AND `gdrive_folder_id` non-empty. Enforced as both `@api.constrains` AND in upload-wizard validation. P1-09.
