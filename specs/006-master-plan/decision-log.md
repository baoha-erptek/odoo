...
| **D-23** | ADR-007 fulfillment-mixin inheritance direction (sale.order ↔ sale.order.fulfillment) — original ADR was internally contradictory: model definition placed `_inherits` on the child, prose described parent-side delegation | Owner + Architect | Before P1-05 code | **RESOLVED** | **Option A (invert direction).** `sale.order._inherits = {'sale.order.fulfillment': 'fulfillment_id'}`; sale.order.fulfillment is a standalone Model with all operational fields; sale.order auto-creates the sibling on every create (covers all sale.order rows DB-wide, accepted as a deliberate cost of multichannel_hub_core being a foundation dep). Matches the ADR's prose intent (`sale.order.tracking_number` Just Works) and Spec 003 dashboard expectations. ADR-007 amended with banner. | Owner, 2026-04-27 | [ADR-007 banner](adrs/ADR-007-fulfillment-delegation-mixin.md); P1-05 slice; tracker `006-master-plan-tracking.md` P1-05 row |
