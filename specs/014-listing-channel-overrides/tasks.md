...
- [ ] T101 Dispatch `planner` agent (Haiku) with spec 014 + ADR-017 as input. Output: file-by-file diff outline. Expected files: `etsy_shop.py` (3 fields), `multichannel_listing_etsy_views.xml` (shop form extend), `etsy_listing_publisher.py` (3 fallback methods), test files.

### Phase 2 — RED

- [ ] T201 `tests/test_p_enh_esty_190_phase1_db.py` — 4 Phase-1 DB assertions per spec §6 (column existence, widths, nullability).
- [ ] T202 `tests/test_p_enh_esty_190_phase2_orm.py` — 8 Phase-2 ORM tests per spec §6 (fallback chain: listing → product → shop for title/description/image; debug logging; end-to-end integration).
- [ ] T203 Verify all 12 tests RED before any code.

### Phase 3 — GREEN

- [ ] T301 Extend `custom_addons/etsy_integration/models/etsy_shop.py`:
...
- [ ] T302 Extend `custom_addons/etsy_integration/models/etsy_listing_publisher.py` (or new service file, if planner recommends):
...
- [ ] T303 Extend view in `custom_addons/etsy_integration/views/etsy_shop_*.xml` (or create new view file):
...
- [ ] T304 Add data file reference to `__manifest__.py` `data` list if new view file created. Bump etsy_integration version `19.0.3.8.0 → 19.0.3.9.0` (point release).
- [ ] T305 Optional: Create `custom_addons/etsy_integration/data/etsy_shop_defaults_demo.xml` seed file with demo shop defaults (if demo data is used). Otherwise, skip (fields default to NULL).
- [ ] T306 Run tests: all 12 GREEN under `--test-tags /etsy_integration`.

### Phase 4 — Review (parallel)

- [ ] T401 `code-reviewer` agent (Haiku) — single Agent call.
- [ ] T402 `security-reviewer` agent (Haiku) — single Agent call (parallel with T401).
...
- [ ] T502 Full suite regression: `--test-tags /etsy_integration` matches baseline 18 fail / 5 error of (728) — zero new regressions.
...
- [ ] T701 Update `docs/owner/HUONG_DAN_QUAN_LY_KENH_BAN.md` §5 (new) — "Cài đặt mặc định tiêu đề/mô tả/hình ảnh cho mỗi cửa hàng Etsy" (Vietnamese walkthrough with screenshots).
- [ ] T702 Update `docs/owner/UAT_WALKTHROUGH_QUAN_LY_KENH_BAN.md` TC-025 (new row) — "Verify per-shop title default" (6-step test case).
- [ ] T703 Update `.claude/plans/006-master-plan-tracking.md` row P-ENH-ESTY-190: `todo → done`, append ship note matching Wave-3 row format.
- [ ] T704 Append Phase 3-9 surprises to `specs/014-listing-channel-overrides/findings.md`.
