...
- [~] T026 [US2] Extend `sale.order` in `models/sale_order.py` — add `sync_source`, `etsy_last_modified`, `etsy_tracking_push_status`, `etsy_tracking_push_at`, `etsy_tracking_push_error` per data-model.md §6 — **partial P0-16c 2026-04-28**: `sync_source` (P0-16b1) + `etsy_last_modified` + `payment_status` landed. The `etsy_tracking_push_*` fields belong to Spec 005 US3 (P1-12); deferred to that slice.
