...
test.describe('UAT Flow-3 MTO — HUONG_DAN_GIAO_HANG_VN §9.MTO', () => {
  ...
  test('TC-MTO-001 — Pipeline initial-state smoke + Operations dashboard row', async ({ page, request }) => {
    ...
    // Operations dashboard surfaces the seeded order.
    const dash = new OperationsDashboardPage(page);
    await dash.open();
    await dash.search(orderRef);
    const rows = await dash.readVisibleRowCount();
    expect(rows, `Operations dashboard lists at least one row for ${orderRef}`).toBeGreaterThan(0);
  });
  ...
  test('TC-MTO-003 — Reject design file shows Rejected column', async ({ page, request }) => {
    ...
    const kanban = new DesignFilesKanbanPage(page);
    await kanban.open();
    await kanban.filterByOrder(orderRef);
  ...
  test('TC-MTO-004 — 10 MB cap rejection', async ({ page, request }) => {
    const orderRef = `${UAT_ORDER_REF_PREFIX}-MTO-001`;
    const orderId = await lookupUatOrderId(request, orderRef);
    test.skip(!orderId, `Fixture order ${orderRef} not seeded`);
    // Build a 12MB random blob in a tmp file (gitignored).
    const tmp = path.join(__dirname, '..', 'fixtures', 'assets', '_tmp_12mb.bin');
    ...
    try {
      ...
      await so.openDesignUploadWizard();
      const kanban = new DesignFilesKanbanPage(page);
      const designName = `UAT-2026-05-31-Design-12MB-${Date.now()}`;
      await kanban.fillUploadWizardWithFile(designName, tmp);
      const result = await kanban.submitUploadWizard();
  ...
  test('TC-MTO-006 — Broken-schema GKE import flagged + needs approve', async ({ page }) => {
...
test.describe('UAT Flow-3 Dropship — HUONG_DAN_GIAO_HANG_VN §9.Dropship', () => {
  ...
  test('TC-DROP-001 — Order with x_gearment_sku routes Dropship', async ({ page, request }) => {
    ...
    const lines = await rpc(request, 'sale.order.line', 'read',
      [so.order_line, ['product_id', 'x_gearment_sku']]);
    const dropLine = lines.find((l: any) => l.x_gearment_sku);
    expect(dropLine, `at least one line on ${orderRef} carries x_gearment_sku`).toBeTruthy();
    expect(dropLine.x_gearment_sku, 'gearment SKU matches seed').toBe('GEAR-UAT-MUG-001');
    // UI sanity: the Gearment quote button is rendered.
    await loginAsAdmin(page);
    ...
    expect(await btn.count(), 'action_open_gearment_quote_wizard button rendered').toBeGreaterThan(0);
  });
