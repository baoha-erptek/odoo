...
SHOTS: list[tuple[str, str, tuple, str | None]] = [
    # HUONG_DAN_DON_HANG_ETSY_VN (flow-2)
    ("don-hang-etsy-shop-form", "form", ("etsy.shop", 10), None),
    ("don-hang-order-list", "action", ("etsy_integration", "action_etsy_orders"), None),
    ("don-hang-order-form", "form", ("sale.order", 3355), None),
    ("don-hang-email-log-list", "action", ("etsy_integration", "action_etsy_email_log"), None),
    ("don-hang-sync-health", "action", ("etsy_integration", "action_etsy_sync_health"), None),
    # HUONG_DAN_GIAO_HANG_VN (flow-3a/3b)
    ("giao-hang-operations-dashboard", "action",
     ("multichannel_hub_core", "action_operations_dashboard"), None),
    ("giao-hang-order-pipeline", "action",
     ("multichannel_hub_core", "action_order_pipeline"), None),
    ("giao-hang-order-form-tracking", "form", ("sale.order", 3355), None),
    ("giao-hang-mo-form", "form", ("mrp.production", 20), None),
    ("giao-hang-design-file-form", "form", ("design.file", 111), None),
    ("giao-hang-tracking-import-log", "action",
     ("multichannel_hub_fulfillment", "action_tracking_import_log"), None),
    ("giao-hang-fulfillment-detail", "action",
     ("multichannel_hub_fulfillment", "action_sale_order_fulfillment_detail"), None),
    # HUONG_DAN_HAU_MAI_VN (flow-4)
    ("hau-mai-ticket-list", "action",
     ("etsy_integration", "action_etsy_order_ticket"), None),
    ("hau-mai-ticket-form", "form", ("etsy.order.ticket", 4), None),
    ("hau-mai-address-change-list", "action",
     ("etsy_integration", "action_etsy_address_change_request"), None),
    ("hau-mai-address-change-form", "form",
     ("etsy.address.change.request", 22), None),
...
def main() -> int:
    ...
    with sync_playwright() as pw:
        ...
        for name, kind, target, extra in SHOTS:
